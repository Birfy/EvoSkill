...
| `--judge_input_cost_per_1m X` / `--judge_output_cost_per_1m X` | Optional direct judge pricing override in USD per 1M tokens. Without these, known OpenAI model prices are used when available. |
...
- Cost accounting split by trajectory, preloaded trajectory, proposer/generator evolution calls, and direct judge API calls. Direct judge calls always record token usage; dollar cost uses known OpenAI model prices when available, with `--judge_input_cost_per_1m` and `--judge_output_cost_per_1m` as overrides.
